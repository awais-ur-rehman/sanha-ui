...
const Products = () => {
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useGetApi<any>(`${PRODUCT_ENDPOINTS.getAll}?${queryParams}`, {
    requireAuth: true,
  ...
  const products =
    productsResponse?.data?.data?.map((product: Product) => ({
      ...product,
  ...
  const handleToggleStatus = async (product: Product, newStatus?: boolean) => {
    try {
      ...
      const statusToSet =
        newStatus !== undefined ? newStatus : product.isActive;
      ...
      const response = await fetch(
        `${API_CONFIG.baseURL}${PRODUCT_ENDPOINTS.update}/${product.id}`,
        {
          method: "PUT",
      ...
      <div className="bg-white rounded-lg overflow-hidden min-h-[calc(100vh-35px)] p-6">
        ...
        >
          <div className="h-[70vh] overflow-hidden">
            <ProductForm
              onSubmit={(data: ProductCreateRequest | ProductUpdateRequest) =>
                handleSubmitCreate(data as ProductCreateRequest)
              }
