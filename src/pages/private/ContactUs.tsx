...
import { FiSearch, FiMessageCircle, FiEdit2, FiChevronDown } from 'react-icons/fi'
...
// Custom dropdown component for ContactUs with first word display and increased height
const ContactUsTypeDropdown = ({ options, value, onChange, placeholder, className = '' }: {
    ...
    <div className="relative w-full" ref={dropdownRef}>
      ...
      >
        <span className={`${selectedOption ? 'text-gray-500' : 'text-gray-900'} pr-8 block`}>
          {selectedOption ? selectedOption.label.split(' ')[0] : placeholder || 'Select an option'}
        </span>
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`
                  w-full px-3 py-3.5 text-left text-xs rounded-md
