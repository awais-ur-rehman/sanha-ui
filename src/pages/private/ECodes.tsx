...
import { useGetApi, useDeleteApi, useBulkImport } from "../../hooks";
...
const ECodes = () => {
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useGetApi<any>(`${ECODE_ENDPOINTS.getAll}?${queryParams}`, {
    requireAuth: true,
  ...
  const handleToggleStatus = async (ecode: ECode, newStatus?: boolean) => {
    try {
      ...
      const response = await fetch(
        `${API_CONFIG.baseURL}${ECODE_ENDPOINTS.update}/${ecode.id}`,
        {
          method: "PUT",
          headers: getAuthHeaders(),
      ...
      if (!response.ok) {
        ...
        if (response.status === 404 && !statusToSet) {
          ...
          if (selectedECode?.id === ecode.id) {
            setSelectedECode((prev) =>
              prev ? { ...prev, isActive: statusToSet } : null
            );
          }
            ...
            <StyledTable<ECode>
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (e: ECode) => (
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${e.status === "Halal"
                ...
                },
                {
                  ...
                  render: (e: ECode) => (
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${e.isActive
                        ? "bg-green-100 text-green-800"
                        : "bg-red-100 text-red-800"
                        }`}
                    >
          ...
          statusBadge={
            selectedECode
              ? {
                ...
                color:
                  selectedECode.status === "Halal"
                    ? "green"
                    : selectedECode.status === "haraam"
                      ? "red"
                      : "yellow",
              }
              : undefined
          ...
          sections={[
            {
              ...
              items: [
                { label: "Code", value: selectedECode?.code || "N/A" },
                {
                  ...
                  value: selectedECode?.alternateName?.join(", ") || "N/A",
                },
              ],
            },
            {
